...
def cobun(e, x, factors, tapes):
    return cobundle(e, x, tape(factors, tapes, 0, 0))

def variable(e, x): return cobun(e, x, [], [])
...
def lift_real_to_real(f, dfdx):
    def me(x):
        if isinstance(x, cobundle):
            return cobun(x.e, me(x.prim), [dfdx(x.prim)], [x.tape])
        else: return f(x)
    return me
    ...
    def me(x1, x2):
        if isinstance(x1, cobundle):
            if isinstance(x2, cobundle):
                if x1.e<x2.e: return cobun(x2.e,
                                           me(x1, x2.prim),
                                           [dfdx2(x1, x2.prim)],
                                           [x2.tape])
                elif x2.e<x1.e: return cobun(x1.e,
                                             me(x1.prim, x2),
                                             [dfdx1(x1.prim, x2)],
                                             [x1.tape])
                else: return cobun(x1.e,
                                   me(x1.prim, x2.prim),
            ...
            if isinstance(x2, cobundle): return cobun(x2.e,
                                                      me(x1, x2.prim),
                                                      [dfdx2(x1, x2.prim)],
                                                      [x2.tape])
            else: return f(x1, x2)
...
def derivative(f):
    def me(x):
        e = generate_epsilon()
        if isinstance(x, list): x_reverse = [variable(e, xi) for xi in x]
        else: x_reverse = variable(e, x)
